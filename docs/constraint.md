...
- **用户前台侧**：登录/注册、智能对话（五区布局+SSE流式）、智能问数（意图识别+数据库查询+报表渲染）、数字员工调用（@菜单+LLM/API双型）、@技能命令（天气/新闻-百度实时抓取/音乐/电影/yummy）、PDF导出、快捷功能（/菜单）
...
### 初始化数据
- **AI模型预设**: 5个（GPT-4o、GPT-4o-turbo、Claude-3.5、DALL-E-3文本生图、Kling视频生视频）
- **数字员工预设**: 6个（采集专员-LLM型、天气-API型、小dummy-LLM型、新闻-API型、随机音乐-API型、电影-API型）
- **瞭源预设**: 3个（百度新闻-news/html、Hacker News-news/json_api、36氪-news/rss）
- **功能数量**: 15个（5个一级 + 10个二级）
- **角色预设**: 2个（系统管理员、普通用户）
- **敏感词预设**: 15条（用户攻击防护）
...
- **管理侧**: 日志审计、系统配置界面、技能管理、接口管理、会话/对话管理、系统设置
- **交互增强**: 人脸登录后端对接、手势识别后端对接、语音合成播报
- **安全治理**: 配置从 `run.py` 抽离到 `config/`、OWASP专项清单

### Q&A
**Q**: `config/` 和 `skills/` 目录为什么是空的？
**A**: `config/` 为配置目录，当前已存放 `api_keys.json`（外部API密钥管理），其余端口/模型配置仍硬编码在 `run.py` 中；`skills/` 为开发技能预留目录。
