...
- 所有配置统一存放在 `config/` 目录下（预留，当前部分配置仍硬编码在 `run.py` 中）。
...
- **用户前台侧**：登录/注册、智能对话（五区布局+SSE流式）、智能问数（意图识别+数据库查询+报表渲染）、数字员工调用（@菜单+LLM/API双型）、@技能命令（天气/新闻/音乐/电影/yummy）、PDF导出、快捷功能（/菜单）
...
**A**: `config/` 为配置预留目录，当前端口/密钥/模型配置仍硬编码在 `run.py` 中；`skills/` 为开发技能预留目录，均为框架预留空间。
